...
class ChordDecoder(nn.Module):
    ...
    def forward(self, z_chd, inference, tfr, c=None):
        ...
        bs = z_chd.size(0)
        z_chd_hid = self.z2dec_hid(z_chd).unsqueeze(0) # [1, bs, hidden_dim]
        z_chd_in = self.z2dec_in(z_chd).unsqueeze(1) # [bs, 1, z_input_dim]
        if inference:
            tfr = 0.
        token = self.init_input.repeat(bs, 1).unsqueeze(1) # [bs, 1, 36]
        recon_root = []
        recon_chroma = []
        recon_bass = []
        ...
        for t in range(int(self.num_step / 4)):
            ...
            r_chroma = self.chroma_out(chd).view(bs, 1, 12, 2).contiguous() # [bs, 1, 12, 2]
            ...
            if teacher_force and not inference:
                ...
                t_root = torch.zeros([bs, 1, 12], device=z_chd.device).float()
                t_root[torch.arange(0, bs), 0, r_root.max(-1)[-1]] = 1.
                t_chroma = r_chroma.max(-1)[-1].float() # [bs, 1, 12]
                
                t_bass = torch.zeros([bs, 1, 12], device=z_chd.device).float()
...
class TextureEncoder(nn.Module):
    ...
    def forward(self, pr):
        ...
        bs = pr.size(0)
        pr = pr.unsqueeze(1) # pr: (bs, 1, 32, 128)
        pr = self.cnn(pr).view(bs, 8, -1) # (bs, num_channel, 8, 29)->(bs, 8, num_channel*29)
        pr = self.fc2(self.fc1(pr))  # (bs, 8, emb_size)
        pr = self.gru(pr)[-1] # (2, batch, hidden_dim)
        pr = pr.transpose_(0, 1).contiguous()
        pr = pr.view(pr.size(0), -1) # (batch, 2*hidden_dim)
...
class PtvaeDecoder(nn.Module):
    ...
    def decode_note(self, note_summary, batch_size):
        ...
        est_durs = torch.zeros([batch_size, self.dur_width, 2], device=self.device)
    ...
    def decode_notes(self, notes_summary, batch_size, notes, inference,
        ...
        predicted_notes = torch.zeros([batch_size, self.max_simu_note,
                                          self.note_emb_size], device=self.device)
    ...
    def decoder(self, z, inference, x, lengths, teacher_forcing_ratio1,
        ...
        pitch_outs = torch.cat(pitch_outs, dim=1) # (B, num_step, max_simu_note-1, pitch_range)
        dur_outs = torch.cat(dur_outs, dim=1) # (B, num_step, max_simu_note-1, dur_width, 2)
    ...
    def forward(self, z, inference, x, lengths, teacher_forcing_ratio1,
                teacher_forcing_ratio2):
        self.dec_time_gru.flatten_parameters()
        self.dec_notes_emb_gru.flatten_parameters()
        self.dec_notes_gru.flatten_parameters()
        self.dec_dur_gru.flatten_parameters()
